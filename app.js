...
function getUserId() {
    let userId = localStorage.getItem('mylo_user_id');
    if (!userId) {
        userId = crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).substring(2) + Date.now().toString(36);
        localStorage.setItem('mylo_user_id', userId);
    }
    return userId;
}
...
async function saveFavoriteToSupabase(channelId) {
    ...
    try {
        const { error } = await supabase
            .from('favorites')
            .insert({ user_id: state.userId, channel_id: channelId });
