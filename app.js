// ============================================
// MYLO-TV APP.JS - SUPABASE INTEGRATED
// ============================================
...
// USER & DATA SYNC
// ============================================
async function initUserAndSync() {
  ...
  state.userFingerprint = fp;

  if (!state.supabase) return;

  try {
    // Upsert user record
    const { data: user, error: userErr } = await state.supabase
      .from('users')
      .upsert({ 
        user_fingerprint: fp, 
    ...
    state.supabase.rpc('increment_user_views', { p_user_id: user.id }).then(({error}) => {
      if(error) console.warn('View increment warning:', error.message);
    });

    // Load favorites from cloud
    const { data: favs, error: favErr } = await state.supabase
      .from('favorites')
      .select('channel_id')
      .eq('user_id', user.id);

    if (!favErr && favs) {
      state.favorites = favs.map(f => ({ id: f.channel_id }));
      console.log(`☁️ Synced ${state.favorites.length} favorites from Supabase`);
    }

    // Update visible stats
    updateVisibleCounter({ totalViews: user.total_views || 0, uniqueUsers: 1 });

  } catch (err) {
    console.error('⚠️ Supabase sync failed, falling back to local:', err);
    // Fallback to local storage if DB fails
    state.favorites = JSON.parse(localStorage.getItem('myloFavorites')) || [];
  }
}

async function trackChannelView(channelId) {
  if (!state.supabase || !state.userId || !channelId) return;
  
  const channel = state.channelsById.get(channelId);
  
  // Fire-and-forget logging
  state.supabase.from('view_logs').insert({
    user_id: state.userId,
    channel_id: channelId,
    viewed_at: new Date().toISOString()
  }).then(({error}) => {
    if(error) console.warn('View log warning:', error.message);
  });

  // Update aggregate stats
  state.supabase.from('channel_stats').upsert({
    channel_id: channelId,
    ...
    if(error) console.warn('Stats update warning:', error.message);
  });
}

async function syncFavoriteToCloud(channelId, action) {
  if (!state.supabase || !state.userId) return;
  
  const channel = state.channelsById.get(channelId);
  
  if (action === 'add') {
    await state.supabase.from('favorites').upsert({
      user_id: state.userId,
      channel_id: channelId,
      channel_name: channel?.name || '',
      country: channel?.country || ''
    ...
    await state.supabase.from('favorites')
      .delete()
...
// ============================================
// CHANNEL SOURCING & OPENVIEW TAGGING
// ============================================
...
async function playWithAntiBlock(channel, streamIndex = 0, attempt = 0) {
  ...
  const tryNextUrl = () => {
    ...
    const onError = () => {
      ...
      } else if (streamIndex + 1 < rotatedStreams.length) {
        playWithAntiBlock(channel, streamIndex + 1, attempt + 1);
      } else {
...
async function fetchWithCache(url, key) {
  ...
  if (cached) { try { return { json: () => Promise.resolve(JSON.parse(cached)) }; } catch(e) {} }
...
function updateStats() {
  ...
  const txt = `${total.toLocaleString()} Channels • ${state.channelsByCountry.size} Countries`;
...
async function toggleFavorite(channelId){
  ...
  if(idx>-1){
    ...
    syncFavoriteToCloud(channelId, 'remove');
    showToast('Removed from favorites');
  } else {
    state.favorites.push({id:channelId});
    syncFavoriteToCloud(channelId, 'add');
...
async function init() {
  try {
    ...
    // Initialize Supabase and User Sync FIRST
    initSupabase();
    await initUserAndSync();
