...
async function getOrCreateUser() {
  let userFingerprint = localStorage.getItem('mylo_user_fp');
  
  if (!userFingerprint) {
    userFingerprint = crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).substring(2);
    localStorage.setItem('mylo_user_fp', userFingerprint);
  }
  
  try {
    // Check if user exists
    const { data: existingUser, error: fetchError } = await state.supabase
      .from('users')
      .select('*')
    ...
    if (existingUser) {
      ...
      // Update last visit and increment views
      await state.supabase
        .from('users')
        .update({ 
          last_visit: new Date().toISOString(),
          total_views: existingUser.total_views + 1
        })
      ...
      // Create new user
      const { data: newUser, error: insertError } = await state.supabase
        .from('users')
        .insert([{
...
async function loadFavoritesFromSupabase() {
  ...
  try {
    ...
    state.favorites = data.map(fav => ({ id: fav.channel_id }));
    console.log(`✅ Loaded ${state.favorites.length} favorites from Supabase`);
...
async function saveFavoriteToSupabase(channelId) {
  ...
  const channel = state.channelsById.get(channelId);
...
// ============================================
async function trackView(channelId) {
  if (!state.userId) return;
...
async function playWithAntiBlock(channel, streamIndex = 0, attempt = 0) {
  ...
  // Track this view in Supabase
  await trackView(channel.id);
...
async function initTracking() {
  try {
    ...
    if (user) {
      // Update visible counter with Supabase data
      updateVisibleCounter({
        totalViews: user.total_views,
        uniqueUsers: 1 // This would need a separate query for global stats
      });
    }
  } catch (e) { 
...
function initParticles() {
  ...
  for (let i = 0; i < 70; i++) {
    particles.push({ x: Math.random() * window.innerWidth, y: Math.random() * window.innerHeight,
      ...
      alpha: Math.random() * 0.5 + 0.1, color: Math.random() > 0.6 ? '#00f0ff' : Math.random() > 0.5 ? '#ff006e' : '#8b5cf6' });
  }
  function draw() {
...
function initGlobe() {
  ...
  const globe = Globe()
    ...
    .polygonsData([]).polygonAltitude(0.007)
...
function initNewFeatures() {
  ...
  if(favToggle&&favSidebar){favToggle.addEventListener('click',async ()=>{
    // Reload favorites from Supabase before showing
    await loadFavoritesFromSupabase();
    favSidebar.classList.remove('hidden');
  ...
  if(pfav){pfav.addEventListener('click',async ()=>{
    if(state.currentChannel){
      await toggleFavorite(state.currentChannel.id);
      updatePlayerFavButton();
    }
  });}
...
async function showToast(msg){const t=$('toast');if(!t)return;t.textContent=msg;t.classList.remove('hidden');t.classList.add('visible');setTimeout(async ()=>{t.classList.remove('visible');setTimeout(()=>t.classList.add('hidden'),300);},3000);}
...
function initGlobalEvents(){
  ...
  document.addEventListener('keydown',e=>{if(e.key==='Escape'){closePlayer();const s=$('sidebar');if(s)s.classList.add('hidden');const fs=$('favorites-sidebar');if(fs)fs.classList.add('hidden');const sr=$('search-results');if(sr)sr.classList.add('hidden');const m=$('donation-modal');if(m){m.classList.remove('visible');setTimeout(()=>m.classList.add('hidden'),300);}}});
}
...
async function init() {
  try {
    ...
    // Initialize Supabase and user tracking
...
# Save to file
with open('/tmp/app_with_supabase.js', 'w') as f:
    f.write(updated_app_js)

print("✅ Updated app.js with Supabase integration has been generated!")
print("\nKey changes made:")
print("1. Added Supabase initialization with your credentials")
print("2. Created user management functions (getOrCreateUser)")
print("3. Added favorites sync with Supabase (load/save/remove)")
print("4. Added view tracking to Supabase")
print("5. Modified initTracking() to use Supabase")
print("6. Updated toggleFavorite() to sync with database")
print("7. Modified playWithAntiBlock() to track views")
print("8. Updated favorites loading to fetch from Supabase")

result = 
# Generate the updated app.js file with Supabase integration
